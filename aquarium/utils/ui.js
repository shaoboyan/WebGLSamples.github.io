...
var Ui = (function() {
  ...
  function segmentPlaneInteracts(segmentBeginPoint, orientation, plane) {
    // 1. Line equation based on orientation quaternion.
  ...
  Menu.prototype.load = function(url) {
    ...
    // Cursor uses head-neck model. Computing view matrix for cursor in advance.
  ...
  Menu.prototype.onload_ = function(data, exception) {
    if (exception) {
      ...
      // Recording metadata of labels with textures and loading these textures.
      for (var index in data.menu.labels) {
        ...
        if (data.menu.labels[index].isSwitch) {
          labelInfo.texture[0] =  tdl.textures.loadTexture(
             './assets/' + data.menu.labels[index].texture[0], true);
          labelInfo.texture[1] = tdl.textures.loadTexture(
            './assets/' + data.menu.labels[index].texture[1], true);
        } else {
          labelInfo.texture[0] = tdl.textures.loadTexture(
             './assets/' + data.menu.labels[index].texture[0], true);
        }
      ...
      // Recording metadata of labels(numbers of fishes) that are rendered by text renderer.
      ...
      for (var index in this.numberOfFish) {
        ...
        // Generate vertices for fish number labels.
  ...
  Menu.prototype.render = function(projectionMat, modelViewMat, orientations) {
    ...
    var numberIndex = this.menu.numberIndex; // id of fish number label.
    var textureIndex = this.menu.textureIndex; // id of label with texture.

    if (this.isMenuMode) {
      // First, rendering numbers of fishes through text renderer.
      ...
      for (var i in numberIndex) {
        ...
        if (index == this.selectedLabelIndex) {
          textMatrix[14] = 0.5; // Selected labels move close to user.
        } else {
          textMatrix[14] = 0;
        }
      ...
      // Then, rendering labels with textures.
      ...
      for (var i in textureIndex) {
        ...
        if (!layout[index].isAdvancedSettings || this.option) {
          ...
          if (index == this.selectedLabelIndex) {
            gl.vertexAttrib4f(this.labelAttribs.modifyMat, 0, 0, 0.5, 0); // Selected labels move close to user.
          } else {
          ...
          if (layout[index].name == this.clickedLabel) {
            if (layout[index].label.isSwitch) {
                layout[index].label.presentTextureId == 0 ?
                  layout[index].label.presentTextureId += 1 :
                  layout[index].label.presentTextureId -= 1;
            }
          } 
  ...
  Menu.prototype.queryClickedLabel = function(segmentBeginPoint, orientation) {
    ...
    var plane = [0, 0, 1, 10]; // Auxiliary plane for finding intersection.
    var selectedPlane = [0, 0, 1, 9.5]; // Auxiliary plane for finding intersection when label is selected.

    // Auxiliary bounds for fast determining if it is within range.
    ...
    if (this.selectedLabelIndex >= 0) {
      ...
      if (layout[this.selectedLabelIndex].vertex[bottomIndex] < selectedInteractPoint[1]
        ...
        // Looking at the label 1.5 seconds to trigger click event.
